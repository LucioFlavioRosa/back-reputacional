...
AREA_POR_FRENTE: dict[str, str] = {
    ...
    "eventos": "comunicacao",
    "governo": "relacoes_institucionais",
    "parceiros": "relacoes_institucionais",
    "legislativo": "relacoes_institucionais",
    "investidores": "relacoes_investidores",
    "bancos_credores": "operacoes_financeiras",
    "interna": "performance_e_dados",
}
...
def vincular_areas(sessao: Session) -> int:
    ...
    pela tela.
    """
    id_da_area = {a.codigo: a.id for a in sessao.scalars(select(AreaPessoa))}
    ...
    for interacao in sessao.scalars(select(InteracaoRegistro)):
        ...
        area_codigo = AREA_POR_FRENTE.get(codigo) if codigo else None
        if area_codigo is None:
            continue
        sessao.add(InteracaoArea(interacao_id=interacao.id, area_id=id_da_area[area_codigo]))
        ligadas += 1
