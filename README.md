...
privilégio](#versão-e-privilégio). 50 tabelas, organizadas em seis assuntos:

| Assunto | Tabelas principais |
|---|---|
| dicionários | `frente`, `status`, `tema`, `esfera`, `clima`, `resultado`, `formato`, `area_pessoa`, `categoria_publico`, `subcategoria_publico`, … |
| stakeholders | `instituicao`, `interlocutor`, `pessoa_aegea` |
| acesso | `papel`, `usuario`, `escopo`, `acesso_log`, trilha de concessão |
| agendas | `interacao` (tabela-mãe) + cinco extensões 1-para-1 por frente, `interacao_interlocutor`, `interacao_origem`, `interacao_area`, `participacao_aegea`, `material` |
...
As 36 migrations (`0001` a `0037`; a `0019` não existe) ficam em
...
- **Área interna da agenda.** `area_pessoa` é a área da Aegea (Comunicação,
  ...
  para `painel_app` e o gatilho de auditoria vieram só na `0037`). O filtro é
...
```bash
python -m pytest        # 689 testes; precisa do Postgres no ar
